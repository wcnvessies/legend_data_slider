...
class RangeSlider(QWidget):
    def __init__(self, parent, field_name, fmin, fmax):
        ...
        QgsMessageLog.logMessage(
            f"Creating Slider for field {self.field_name}",
            "Slider Plugin",
            level=Qgis.Info,
        )
    ...
    def pretty(self, slider_num):
        num = (float(slider_num) / self.slider.max()) * (
            self.fmax - self.fmin
        ) + self.fmin
        ...
        if self.fmax == self.fmin:
            ...
            return (
                str(int(self.fmax)) if self.fmax > 10 else "{0:.2f}".format(self.fmax)
            )
        pretty_out = ""
    ...
    def eventFilter(self, source, event):
        ...
        if event.type() == QtCore.QEvent.ContextMenu and source is self:
            menu = QMenu()
            menu.addAction(f"Remove {self.field_name}")
    ...
    def getRangeFilter(self):
        ...
        filter_clause1 = f'"{self.field_name}" >= {start_actual_val}'
        filter_clause2 = f'"{self.field_name}" <= {end_actual_val}'
        return f"{filter_clause1} AND {filter_clause2}"

    def on_value_changed(self):
        if self._dirty == False:
            QgsMessageLog.logMessage(
                f"Switching field {self.field_name} to dirty",
                "Slider Plugin",
                level=Qgis.Info,
            )
            self._dirty = True
...
class DataLayerRangeFilterWidget(QWidget):
    ...
    def __init__(self, layer):
        ...
        slider_names = self.layer.customProperty(
            WIDGET_SETTING_PREFIX % SLIDER_LIST_CONFIG_NAME, None
        )
        if slider_names is not None:
            ...
            for field in db.fields():
                QgsMessageLog.logMessage(
                    f"Adding slider for field {field.name()}",
                    "SLider Plugin",
                    level=Qgis.Warning,
                )
                self._add_slider(field.name())
        QgsMessageLog.logMessage(
            "DONE adding sliders", "Range Filter Plugin", level=Qgis.Warning
        )
        self._save_sliders()
    ...
    def _save_sliders(self):
        ...
        self.layer.setCustomProperty(
            WIDGET_SETTING_PREFIX % SLIDER_LIST_CONFIG_NAME, "###".join(slider_names)
        )
    ...
    def _add_slider(self, field_name):
        ...
        if i != -1:
            field = db.fields()[i]
            if field.isNumeric():
                field_max = self.layer.aggregate(
                    QgsAggregateCalculator.Max, field.name()
                )[0]
                field_min = self.layer.aggregate(
                    QgsAggregateCalculator.Min, field.name()
                )[0]
                ...
                try:
                    ...
                    QgsMessageLog.logMessage(
                        f"Error for fieldname {field_name}: {str(v)}",
                        "Slider Plugin",
                        level=Qgis.Warning,
                    )
    ...
    def on_slider_changed(self, the_slider):
        text = " AND ".join(
            [x for x in map(RangeSlider.getRangeFilter, self.sliders) if x != ""]
        )
        db = self.layer.dataProvider()
